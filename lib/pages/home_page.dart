...
import 'admin_page.dart'; // NEW admin page
...
class _HomePageState extends State<HomePage> with TickerProviderStateMixin {
  ...
  String? userRole; // user role: user | admin | blocked | blocked_lb
  ...
  void _setupTabs() {
    // Play & Leaderboard always exist; Admin tab only if role == 'admin'
    tabs = [
      const Tab(text: 'Play'),
    ...
    if (userRole == 'admin') {
      // Add Admin tab at the end
      tabs.add(const Tab(text: 'Admin'));
      tabViews.add(const AdminPage());
    }

    // re-create TabController with new length
  ...
  Future<void> _loadUsernameAndRole() async {
    ...
    final res = await auth.supabase.from('users').select('username,role').eq('id', userId).maybeSingle();
    setState(() {
        ...
        : Column(
            ...
            children: [
              ...
              SizedBox(
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (_, index) {
                    ...
                    final enabled = unlocked.contains(lvl);
              ...
              Builder(builder: (context) {
                final disabledBecauseBlocked = userRole == 'blocked';
                return ElevatedButton(
                  onPressed: (!unlocked.contains(selectedLevel) || disabledBecauseBlocked)
                      ? null
                      : () async {
                          // If user is blocked_lb, we still allow playing; leaderboard logic will skip.
                  ...
                  child: Center(
                    child: disabledBecauseBlocked
                        ? const Text('Account blocked — cannot play')
              ...
              return Container(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(entry['users']['username'] ?? 'Unknown', style: const TextStyle(fontSize: 16)),
                    ),
                    Text(displayTime, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                  ],
                ),
              );
            },
  ...
  Widget build(BuildContext context) {
    return AnimatedGradientBackground(
      child: Scaffold(
        ...
        appBar: AppBar(
          title: Text('Hello, $username 👋', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 24)),
          centerTitle: true,
          backgroundColor: Colors.transparent,
          elevation: 0,
          actions: [
