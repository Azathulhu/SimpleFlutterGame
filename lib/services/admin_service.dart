...
class AdminService {
  ...
  Future<List<Map<String, dynamic>>> fetchQuestions({String? difficulty}) async {
    // Build the query step-by-step to avoid type inference issues
    var builder = supabase.from('questions').select();
  ...
  Future<void> setLeaderboardBan(String userId, bool banned) async {
    // Add/Update a column 'banned_from_leaderboard' on users table (see SQL in canvas doc)
